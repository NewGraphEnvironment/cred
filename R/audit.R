...
#' Extract all paragraph text from HTML files in a directory
#' @noRd
.html_paragraphs <- function(html_dir) {
  html_files <- list.files(html_dir, pattern = "\\.html$", full.names = TRUE)
  if (length(html_files) == 0L) return(character(0))
  unlist(lapply(html_files, function(f) {
    doc <- xml2::read_html(f)
    nodes <- xml2::xml_find_all(doc, "//p")
    xml2::xml_text(nodes, trim = TRUE)
  }))
}

#' Split paraphrase into frame parts around inline R spans
#' @noRd
.frame_parts <- function(paraphrase) {
  parts <- stringr::str_split(paraphrase, "`r [^`]+`")[[1]]
  # Strip @citekeys and markdown punctuation
  parts <- vapply(parts, function(p) {
    p <- gsub("@[A-Za-z][A-Za-z0-9_:./-]+", "", p)
    p <- gsub("[*_`#\\[\\]()|]", "", p)
    p <- gsub("[{}]", "", p)
    trimws(p)
  }, character(1L), USE.NAMES = FALSE)
  parts[nchar(parts) >= 5L]
}

#' Find HTML paragraph containing all frame parts
#' @noRd
.match_frame_to_html <- function(frame_parts, html_texts) {
  if (length(frame_parts) == 0L) return(NA_character_)
  for (p_text in html_texts) {
    all_found <- all(vapply(frame_parts, function(fp) {
      grepl(fp, p_text, fixed = TRUE)
    }, logical(1L)))
    if (all_found) return(p_text)
  }
  NA_character_
}

#' Replace inline R spans with values extracted from matched HTML paragraph
#' @noRd
.extract_rendered_values <- function(paraphrase, html_match) {
  if (is.na(html_match)) return(paraphrase)

  # Get the inline R spans in order
  spans <- stringr::str_extract_all(paraphrase, "`r [^`]+`")[[1]]
  if (length(spans) == 0L) return(paraphrase)

  # Split paraphrase into frame parts (including short ones for positional use)
  raw_parts <- stringr::str_split(paraphrase, "`r [^`]+`")[[1]]
  # Clean frame parts for searching in HTML
  clean_part <- function(p) {
    p <- gsub("@[A-Za-z][A-Za-z0-9_:./-]+", "", p)
    p <- gsub("[*_`#\\[\\]()|]", "", p)
    p <- gsub("[{}]", "", p)
    p
  ...
  for (i in seq_along(spans)) {
    # Find anchors: text before and after this span
    before <- clean_part(raw_parts[i])
    after  <- if (i < length(raw_parts)) clean_part(raw_parts[i + 1L]) else ""

    # Trim before to last 30 chars for precision
    if (nchar(before) > 30L) {
      before <- substr(before, nchar(before) - 29L, nchar(before))
    }
    # Trim after to first 30 chars
    if (nchar(after) > 30L) after <- substr(after, 1L, 30L)

    # Find position of before-anchor in HTML
    before_trimmed <- trimws(before)
    after_trimmed  <- trimws(after)

    if (nchar(before_trimmed) > 0L && nchar(after_trimmed) > 0L) {
      b_pos <- regexpr(before_trimmed, html_match, fixed = TRUE)
      if (b_pos > 0L) {
        start <- b_pos + attr(b_pos, "match.length")
        a_pos <- regexpr(after_trimmed, substr(html_match, start, nchar(html_match)),
                         fixed = TRUE)
        if (a_pos > 0L) {
          val <- substr(html_match, start, start + a_pos - 2L)
          val <- trimws(val)
          result <- stringr::str_replace(result, stringr::fixed(spans[i]), val)
        }
      }
    } else if (nchar(before_trimmed) > 0L) {
      # Last span — no after anchor
      b_pos <- regexpr(before_trimmed, html_match, fixed = TRUE)
      if (b_pos > 0L) {
        start <- b_pos + attr(b_pos, "match.length")
        # Take up to next space or punctuation
        remainder <- substr(html_match, start, nchar(html_match))
        val <- stringr::str_extract(remainder, "^[^\\s,;.]+")
        if (!is.na(val)) {
          result <- stringr::str_replace(result, stringr::fixed(spans[i]),
                                         trimws(val))
        }
      }
    }
...
#' Backfill paraphrase_eval from rendered bookdown HTML
#'
#' Paraphrases containing inline R expressions (`` `r expr` ``) cannot be
#' scored accurately because the R variables are only available during
#' bookdown rendering. This function reads the rendered HTML output,
#' matches each paraphrase to its rendered paragraph, and extracts the
#' evaluated values to populate `paraphrase_eval`.
#'
#' @param audit_file `character(1)` path to the audit CSV.
#' @param html_dir `character(1)` path to the rendered HTML output directory
#'   (e.g., from `yaml::read_yaml("_bookdown.yml")$output_dir`).
#' @param overwrite `logical(1)` if `TRUE`, re-evaluate rows that already have
#'   `paraphrase_eval`. Default `FALSE`.
#' @return The updated audit data frame, invisibly. Writes to `audit_file`.
#' @seealso [crd_aud_eval_inline()] for evaluating inline R in an R session,
#'   [crd_aud_score()] which uses `paraphrase_eval` for scoring.
#' @export
#' @examples
#' \dontrun{
#' yml <- yaml::read_yaml("_bookdown.yml")
#' crd_aud_eval_html("qa/citation_audit.csv", html_dir = yml$output_dir)
#' }
crd_aud_eval_html <- function(audit_file,
                               html_dir,
                               overwrite = FALSE) {
  chk::chk_file(audit_file)
  chk::chk_dir(html_dir)
  chk::chk_flag(overwrite)
  ...
  needs_eval <- is.na(d$paraphrase_eval) | overwrite
  needs_eval <- needs_eval & stringr::str_detect(d$paraphrase, stringr::fixed("`r "))
  needs_eval[is.na(d$paraphrase)] <- FALSE

  n_target <- sum(needs_eval, na.rm = TRUE)
  if (n_target == 0L) {
    message("No rows with inline R expressions to evaluate from HTML.")
    return(invisible(d))
  }

  html_texts <- .html_paragraphs(html_dir)
  if (length(html_texts) == 0L) {
    warning("No HTML paragraphs found in ", html_dir, call. = FALSE)
    return(invisible(d))
  }

  n_ok <- 0L
  for (i in which(needs_eval)) {
    fp <- .frame_parts(d$paraphrase[i])
    if (length(fp) == 0L) next

    html_match <- .match_frame_to_html(fp, html_texts)
    if (is.na(html_match)) {
      warning("No HTML match for row ", i, " (", d$citation_key[i], ")",
              call. = FALSE)
      next
    }

    resolved <- .extract_rendered_values(d$paraphrase[i], html_match)
    # Only count as resolved if we actually replaced something
    if (!identical(resolved, d$paraphrase[i])) {
      d$paraphrase_eval[i] <- resolved
      n_ok <- n_ok + 1L
    }
  }

  # Rows without inline R get paraphrase_eval = paraphrase
  no_inline <- !stringr::str_detect(d$paraphrase, stringr::fixed("`r ")) &
    is.na(d$paraphrase_eval)
  no_inline[is.na(d$paraphrase)] <- FALSE
  d$paraphrase_eval[no_inline] <- d$paraphrase[no_inline]

  n_fail <- n_target - n_ok
  message(
    "Resolved inline R from HTML in ", n_ok, " row(s)",
    if (n_fail > 0L) paste0(" (", n_fail, " could not be matched)") else ""
  )
